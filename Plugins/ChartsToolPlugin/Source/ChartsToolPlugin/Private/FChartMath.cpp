...
void FChartMath::GetArrayRange(const TArray<FVector2D>& DataArray, FVector2D& OutMin, FVector2D& OutMax)
{
	...
	for (int i = 0; i < DataArray.Num(); i++)
	{
		...
		// 4. 使用 FMath::Min/Max,
		OutMin.X = FMath::Min(OutMin.X, Temp.X);
		OutMax.X = FMath::Max(OutMax.X, Temp.X);
		OutMin.Y = FMath::Min(OutMin.Y, Temp.Y);
		OutMax.Y = FMath::Max(OutMax.Y, Temp.Y);
	}
}
                                  ...
                                  const FChartAxisSettings& InSettings)
{
	...
	// 计算数据跨度（防止除以 0）
	float RangeX = (InSettings.XAxisMax - InSettings.XAxisMin);
	float RangeY = (InSettings.YAxisMax - InSettings.YAxisMin);
