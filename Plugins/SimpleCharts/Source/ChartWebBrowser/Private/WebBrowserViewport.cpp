...
bool FWebBrowserViewport::IsEditor()
{
	if (GEngine)
	{
...
void FWebBrowserViewport::Tick( const FGeometry& AllottedGeometry, double InCurrentTime, float DeltaTime )
{
	if (!bIsPopup)
	{
		...
		if (IsEditor())
		{
			FVector2D AbsoluteSize = AllottedGeometry.GetLocalSize();
			WebBrowserWindow->SetViewportSize(AbsoluteSize.IntPoint(), FIntPoint(0, 0));
		}
		else
		{
			FVector2D AbsoluteSize = AllottedGeometry.GetLocalSize() * DPIScale;
