	...
	switch (GetLastNotation())
	{
		case EJsonNotation::String:
		{
			if (Property->IsA<FStructProperty>())
			{
				...
				if ( StructProperty->Struct == FChartWebJSFunction::StaticStruct())
				{
					...
					FChartWebJSFunction CallbackObject(Scripting, CallbackID);
					return SetPropertyValue<FStructProperty, FChartWebJSFunction>(Property, Outer, Data, ArrayIndex, CallbackObject);
				}
			}
		}
		break;
