...
namespace
{
	...
	bool WriteJsParam(FMobileJSScriptingRef Scripting, FJsonWriterRef Writer, const KeyType& Key, FChartWebJSParam& Param)
	{
		switch (Param.Tag)
		{
			case FChartWebJSParam::PTYPE_NULL:
				WriteNull(Writer, Key);
				break;
			case FChartWebJSParam::PTYPE_BOOL:
				WriteValue(Writer, Key, Param.BoolValue);
				break;
			case FChartWebJSParam::PTYPE_DOUBLE:
				WriteValue(Writer, Key, Param.DoubleValue);
				break;
			case FChartWebJSParam::PTYPE_INT:
				WriteValue(Writer, Key, Param.IntValue);
				break;
			case FChartWebJSParam::PTYPE_STRING:
				WriteValue(Writer, Key, Param.StringValue);
				break;
			case FChartWebJSParam::PTYPE_OBJECT:
			{
			...
			case FChartWebJSParam::PTYPE_STRUCT:
			{
			...
			case FChartWebJSParam::PTYPE_ARRAY:
			{
			...
			case FChartWebJSParam::PTYPE_MAP:
			{
	...
	for (TFieldIterator<UFunction> FunctionIt(Class, EFieldIteratorFlags::IncludeSuper); FunctionIt; ++FunctionIt)
	{
		...
		for ( TFieldIterator<FProperty> It(Function); It; ++It )
		{
			...
			if (Param->PropertyFlags & CPF_Parm && ! (Param->PropertyFlags & CPF_ReturnParm) )
			{
				...
				if (!StructProperty || !StructProperty->Struct->IsChildOf(FChartWebJSResponse::StaticStruct()))
				{
					if(!firstArg)
					{
						Result.Append(TEXT(", "));
...
void FMobileJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FChartWebJSParam Arguments[], bool bIsError)
{
	TSharedPtr<IWebBrowserWindow> Window = WindowPtr.Pin();
	if (Window.IsValid())
	{
...
{
	FChartWebJSParam Args[1] = {FChartWebJSParam(Error)};
...
bool FMobileJSScripting::HandleExecuteUObjectMethodMessage(const TArray<FString>& MessageArgs)
{
	...
	if (PromiseParam)
	{
		FChartWebJSResponse* PromisePtr = PromiseParam->ContainerPtrToValuePtr<FChartWebJSResponse>(Params.GetData());
		if (PromisePtr)
		{
			*PromisePtr = FChartWebJSResponse(SharedThis(this), ResultCallbackId);
		}
	}
