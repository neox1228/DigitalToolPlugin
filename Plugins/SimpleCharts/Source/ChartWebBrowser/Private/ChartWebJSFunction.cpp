...
FChartWebJSParam::~FChartWebJSParam()
{
...
FChartWebJSParam::FChartWebJSParam(const FChartWebJSParam& Other)
	: Tag(Other.Tag)
{
	switch (Other.Tag)
	{
		...
		case PTYPE_STRUCT:
			...
			break;
		case PTYPE_ARRAY:
			ArrayValue = new TArray<FChartWebJSParam>(*Other.ArrayValue);
			break;
		case PTYPE_MAP:
			MapValue = new TMap<FString, FChartWebJSParam>(*Other.MapValue);
			break;
	}
}

FChartWebJSParam::FChartWebJSParam(FChartWebJSParam&& Other)
	: Tag(Other.Tag)
{
	switch (Other.Tag)
	{
...
void FChartWebJSCallbackBase::Invoke(int32 ArgCount, FChartWebJSParam Arguments[], bool bIsError) const
{
