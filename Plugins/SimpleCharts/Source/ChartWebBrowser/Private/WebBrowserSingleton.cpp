...
#include "Runtime/WebBrowser/Private/Android/AndroidWebBrowserWindow.h"
#include "Runtime/WebBrowser/Private/Android/AndroidCookieManager.h"
#elif PLATFORM_IOS
#include "Runtime/WebBrowser/Private/IOS/IOSPlatformWebBrowser.h"
#include "Runtime/WebBrowser/Private/IOS/IOSCookieManager.h"
#elif PLATFORM_PS4
...
FString FChartWebBrowserSingleton::ApplicationCacheDir() const
{
#if PLATFORM_MAC
...
class FWebBrowserWindowFactory
	: public IChartWebBrowserWindowFactory
{
public:
...
FChartWebBrowserSingleton::FChartWebBrowserSingleton(const FWebBrowserInitSettings& WebBrowserInitSettings)
#if WITH_CEF3
	...
	, DefaultTranslucentMaterial(nullptr)
{
	...
	CEFBrowserApp->OnRenderProcessThreadCreated().BindRaw(this, &FChartWebBrowserSingleton::HandleRenderProcessCreated);
	...
	//}
	Settings.remote_debugging_port = 56231;
	...
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("Chromium Resources information not found at: %s."), *ResourcesPath);
	}
	...
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("Chromium Locales information not found at: %s."), *LocalesPath);
	}
	...
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("UnrealCEFSubProcess.exe not found, check that this program has been built and is placed in: %s."), *SubProcessPath);
	}
...
#if WITH_CEF3
void FChartWebBrowserSingleton::HandleRenderProcessCreated(CefRefPtr<CefListValue> ExtraInfo)
{
...
FChartWebBrowserSingleton::~FChartWebBrowserSingleton()
{
#if WITH_CEF3
	{
		FScopeLock Lock(&WindowInterfacesCS);
...
TSharedRef<IChartWebBrowserWindowFactory> FChartWebBrowserSingleton::GetWebBrowserWindowFactory() const
{
	return WebBrowserWindowFactory;
}

TSharedPtr<IWebBrowserWindow> FChartWebBrowserSingleton::CreateBrowserWindow(
	TSharedPtr<FCEFWebBrowserWindow>& BrowserWindowParent,
...
{
	FChartCreateBrowserWindowSettings Settings;
...
TSharedPtr<IWebBrowserWindow> FChartWebBrowserSingleton::CreateBrowserWindow(const FChartCreateBrowserWindowSettings& WindowSettings)
{
	...
	if (AllowCEF)
	{
		...
		{
			const FChartBrowserContextSettings Context = WindowSettings.Context.GetValue();
...
#if BUILD_EMBEDDED_APP
TSharedPtr<IWebBrowserWindow> FChartWebBrowserSingleton::CreateNativeBrowserProxy()
{
...
bool FChartWebBrowserSingleton::Tick(float DeltaTime)
{
...
FString FChartWebBrowserSingleton::GetCurrentLocaleCode()
{
	FCultureRef Culture = FInternationalization::Get().GetCurrentCulture();
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void FChartWebBrowserSingleton::DeleteBrowserCookies(FString URL, FString CookieName, TFunction<void(int)> Completed)
{
...
TSharedPtr<IWebBrowserCookieManager> FChartWebBrowserSingleton::GetCookieManager(TOptional<FString> ContextId) const
{
	if (ContextId.IsSet())
	{
#if WITH_CEF3
		...
		{
			UE_LOG(LogChartWebBrowser, Log, TEXT("No cookie manager for ContextId=%s.  Using default cookie manager"), *ContextId.GetValue());
		}
#endif
	}
...
bool FChartWebBrowserSingleton::RegisterContext(const FChartBrowserContextSettings& Settings)
{
#if WITH_CEF3
...
bool FChartWebBrowserSingleton::UnregisterContext(const FString& ContextId)
{
#if WITH_CEF3
...
bool FChartWebBrowserSingleton::RegisterSchemeHandlerFactory(FString Scheme, FString Domain, IWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
#if WITH_CEF3
...
bool FChartWebBrowserSingleton::UnregisterSchemeHandlerFactory(IWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
#if WITH_CEF3
