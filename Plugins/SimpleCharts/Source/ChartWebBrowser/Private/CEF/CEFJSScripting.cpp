...
bool FCEFJSScripting::HandleExecuteUObjectMethodMessage(CefRefPtr<CefListValue> MessageArguments)
{
	...
	if (PromiseParam)
	{
		FChartWebJSResponse* PromisePtr = PromiseParam->ContainerPtrToValuePtr<FChartWebJSResponse>(Params);
		if (PromisePtr)
		{
...
void FCEFJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FChartWebJSParam Arguments[], bool bIsError)
{
	CefRefPtr<CefListValue> FunctionArguments = CefListValue::Create();
